...
import {LMap, LTileLayer, LMarker, LPolyline, LPolygon, LCircle} from "@vue-leaflet/vue-leaflet";
...
<template>
  <div class="map-block">
    <l-map @click="click" v-model:zoom="zoom" :center="markers[0] "  :use-global-leaflet="false">
      ...
      <l-polyline :lat-lngs="polyLine" fill-color="#6265F0" :fill-opacity="0.2" :color="`#6265F0`"/>
      <l-polygon :lat-lngs="polygon" color="#6265F0" :fill-opacity="0.2" fill-color="#6265F0"/>
      <l-circle v-for="c in circle" :key="`circleKey${Math.random()}`"
          :lat-lng="c.center"
          :radius="c.radius || 2"

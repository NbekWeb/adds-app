...
import OrderListItemComponent from '@/pages/dashboard/order/kiosk-order/[id]/components/OrderListItemComponent.vue'
...
const { orders, page, totalPages } = storeToRefs(orderPinia)
  ...
  <a-spin class="loader" :spinning="loadingUrl.has('get/kiosk-order/all')">
    ...
    >
      <template #content>
        <template
          v-if="
            !orders?.length && !corePinia.loadingUrl.has('get/kiosk-order/all')
          "
        >
          <a-empty class="empty">
